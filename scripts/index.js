...
const body = document.body; // Assuming the wish will be added to the body
...
x = setInterval(function() {
  ...
  let w = (c.width = window.innerWidth),
    ...
    hh = h / 2,
    opts = {
      strings: ['HAPPY', 'BIRTHDAY!', config.name, config.wish ],  // wish is added here
      charSize: 30,
      charSpacing: 35,
      lineHeight: 40,
  ...
  function Letter(char, x, y) {
    ...
    // Apply a different style for the 'wish' text (smaller and no color)
    if (opts.strings[3].includes(char)) {  // Check if the char is part of the 'wish' text
      this.color = '#000';  // Set the color to black or any color you prefer
      this.lightAlphaColor = 'rgba(0, 0, 0, 0.5)';
      this.lightColor = '#000';
      this.alphaColor = '#000';
      ctx.font = '18px Verdana'; // Set the smaller font size only for wish
    } else {
      this.color = 'hsl(hue,80%,50%)'.replace('hue', hue);
      this.lightAlphaColor = 'hsla(hue,80%,light%,alp)'.replace('hue', hue);
      this.lightColor = 'hsl(hue,80%,light%)'.replace('hue', hue);
